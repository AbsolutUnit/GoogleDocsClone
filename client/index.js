...
function update(delta) {
  ...
  if (delta) {
    // delta is the new change operation
...
docbtn.onclick = (e) => {
  fetch(getUrl).then((res) => {
    res.text().then(function (text) {
      ...
      //quill.root.innerHTML = text // this is sucky
      //quill.root.innerHTML = ''; // clear board
      //quill.clipboard.dangerouslyPasteHTML(0, text); //then paste
    });
  });
};
